...
- `src/hooks/useAutoPickLogic.js` - Extracted auto-pick hook. Manages both auto-pick effects (timer expiry + immediate pick). Internally tracks `lastAutoPickKeyRef`, `prevCurrentPickRef`, `prevAutoPickModeRef` to prevent duplicate fires and mount-time false triggers. Effect 2 also fires when the auto-pick mode changes mid-turn (deps: `draftSettings?.autoPickFromQueue`, `draftSettings?.autoPickGeneral`).
