...
- `src/utils/epCalc.js` - **Single source of truth for EP formula.** `calculateEP(winProbability)` — positional probability model (80/50/30/20). Imported by `oddsApi.js` (re-exported for back-compat) and `oddsScraper.js`. No upstream dependencies — safe to import anywhere.
- `src/utils/multiEventScoring.js` - **Single source of truth for Golf/Tennis per-event scoring.** Exports `golfEventPoints`, `tennisEventPoints`, `computeMultiEventRankings`. Imported by `resultsApi.js` and `points.js`.
...
- `src/hooks/useAutoPickLogic.js` - Extracted auto-pick hook. Manages both auto-pick effects (timer expiry + immediate pick). Internally tracks `lastAutoPickKeyRef`, `prevCurrentPickRef` to prevent duplicate fires and mount-time false triggers. Effect 2 fires when `currentPick` advances or the queue size changes (item added while on the clock) — toggling auto-pick settings mid-turn does NOT immediately fire; the new setting takes effect on the next turn.
...
- `src/useSupabase.js` - React hooks: `useAuth`, `useLeagues`, `useDraft`. `useDraft` re-syncs on `visibilitychange` (tab focus) and on Supabase channel recovery (`TIMED_OUT`/`CHANNEL_ERROR`/`CLOSED`). `useLeagues` also re-syncs on `visibilitychange`.
...
- `src/useTeamPerformance.js` - Hook: `useTeamPerformance(sport, team, selectedSeason = null)` → `{ performance, loading, error }`. When `selectedSeason` is provided (SPORT_SEASONS year convention), fetches that exact season; otherwise auto-selects the most recently completed season. `END_YEAR_SPORTS = ['NBA','NHL','NCAAMB','UCL']` subtract 1 to convert ESPN end-year to DB start-year. Returns `{ type: 'single'|'multi'|'f1', season, result, isComplete, ... }`. Used in TeamPopup Performance tab — season selector drives this hook. Race condition guard: tags each fetch with `requestedSeason` and discards out-of-order responses.
...
- `database/database-migration-draft-manual-pick-hold.sql` - Migration for `manual_pick_paused_until` on `draft_state` (legacy; client now handles rollback hold without a time-based column)
...
- `supabase/functions/auto-pick-from-queue/index.ts` - Edge Function: **server-side auto-pick**. Triggered by a Supabase database webhook on `draft_state` UPDATE. When `current_pick` advances, checks the picker's mode: `auto_pick_from_queue` (queue-only) or `auto_pick_general` (queue first, then EP fallback) → enforces sport coverage rules → advances `draft_state` only if a pick is made → sends OTC email. Cascades naturally through consecutive auto-pick-enabled pickers. Race-safe: unique constraint on `(league_id, pick_number)` in `draft_picks` silently discards duplicate inserts (code `23505`). Requires `APP_URL` secret for email links. Webhook setup: Supabase Dashboard → Database → Webhooks → Table: `draft_state`, Event: Update, URL: `{SUPABASE_URL}/functions/v1/auto-pick-from-queue`, Header: `Authorization: Bearer {SERVICE_ROLE_KEY}`.
...
- `supabase/functions/_shared/draft-helpers.ts` - Shared Deno module: `getPickerIndex`, `normalizeDraftPicker`, `timerStringToMs`, `computeTimeRemaining` (pause-aware), `sendEmail`, `escapeHtml`, `wouldBreakSportCoverage`, `buildCandidates`, `getQueueAutopick`, `fetchExpectedPoints`
...
- Custom hooks:
  ...
  - `useDraftQueue(leagueId, userEmail)` — manages a user's personal draft queue and per-league draft settings. Returns `{ queue, settings, loading, error, addItem, removeItem, moveItem, reorderAll, clearAll, updateSettings, reload }`. `queue` is sorted by `position` ascending. `settings = { autoPickFromQueue: bool, autoPickGeneral: bool }` (mutually exclusive). Mounted in `omnifantasy-app.jsx` and props threaded through to `DraftView`. Note: `receiveOtcEmails` is a **global** preference stored in `user_metadata` (not per-league), managed via `updateUserMetadata()` from `supabaseClient.js`.
...
4. **draft_state** - `league_id`, `current_pick`, `current_round`, `draft_order`, `is_snake`, `third_round_reversal`, `draft_every_sport_required`, `pick_started_at`, `manual_pick_paused_until` (legacy; unused by client), `updated_at`
...
8. **draft_member_settings** - `(league_id, user_email)` PK, `auto_pick_from_queue` (bool, default false), `auto_pick_general` (bool, default false) — per-user per-league draft preferences (mutually exclusive). RLS: SELECT = any authenticated; INSERT/UPDATE = own rows only. Note: `receive_otc_emails` moved to Supabase `user_metadata` (global across leagues) — accessed via `currentUser.user_metadata.receive_otc_emails` and updated with `updateUserMetadata({ receive_otc_emails: bool })`.
...
- `calculateEP(p)` imported from `src/utils/epCalc.js` — single source of truth, no duplication.
...
onUpdateDraftSettings({ autoPickFromQueue: bool, autoPickGeneral: bool, receiveOtcEmails: bool })
...
15. `database/database-migration-draft-manual-pick-hold.sql` — Adds `manual_pick_paused_until` to `draft_state` (legacy; rollback hold is now client-side without time-based column)
16. `database/database-migration-draft-reminders.sql` — Creates `draft_reminders` table + OTC reminder RPC + pg_cron comment
17. `database/database-migration-draft-member-autopick-general.sql` — Adds `auto_pick_general` to `draft_member_settings`
...
- **Auto-pick mid-turn fix** — changing auto-pick mode while it's already your turn triggers immediate pick (Effect 2 watches `draftSettings?.autoPickFromQueue` and `draftSettings?.autoPickGeneral`)
...
- **Auth hydration loading screen** — While `authLoading` is true (Supabase resolving existing session, ~100-300ms), a neutral screen (logo + bouncing dots) is shown so returning users never see a login form flash. This ensures clean UX when opening the app from OTC email links.
- **Deep-link via `?draft=leagueId`** — OTC/reminder emails include `?draft=<leagueId>`. `omnifantasy-app.jsx` captures param in `pendingDraftLeagueIdRef` at mount (persists across effect re-runs). After leagues load and user is confirmed as a member, navigates directly to draft view (or league view if draft not started). Ref is cleared only immediately before navigation to avoid lost-ref bugs.
- **1-hour reminder emails include `?draft=` param** — `check-timer-reminders` Edge Function adds `?draft=<leagueId>` to the reminder email link so users deep-link straight into the draft room.
- **Draft state re-sync on tab focus / channel reconnect** — `useDraft` registers a `visibilitychange` listener (calls `loadDraft()` when tab becomes visible) and a Supabase channel `statusCallback` (calls `loadDraft()` on `TIMED_OUT`/`CHANNEL_ERROR`/`CLOSED`). `useLeagues` registers a matching `visibilitychange` listener. Prevents stale picks/state after backgrounding a tab or mid-session WebSocket drops.
- **Draft button in TeamPopup** — When `TeamPopup` is opened from DraftView, an `onDraft` prop is passed and a compact **Draft** button appears in the popup header alongside the close button. Button is disabled (label "Not your turn") when it is not the user's own turn, the draft is complete, the team is already picked, or the sport slot is full. `draftDisabled` computed in DraftView using `isMyTurn`.
- **`visibleQueue` in DraftView** — `visibleQueue` useMemo filters out already-drafted items from the queue. Used for desktop sidebar queue count badge and empty-state check so the count drops immediately when any pick is made (including by other users).
